...
class PtpIpConnection(object):
    ...
    def open(self, host='192.168.1.1', port=15740):
        ...
        # 0x1002 OpenSession
        ptip_cmd = PtpIpCmdRequest(cmd=0x1002, param1=struct.unpack('<I', self.session_id)[0])
    ...
    def connect(self, host='192.168.1.1', port=15740):
        try:
            ...
            s.connect((host, port))
        except Exception as e: #socket.error: #as (value, message):
            if s:
                s.close()
...
class PtpIpInitCmdReq(PtpIpPacket):
    ...
    def __init__(self, data=None):
        ...
        if data is None:
            ...
            self.hostname = '2532833890.kn.vutbr.cz'+'\x00' #socket.gethostname() + '\x00'
...
class PtpIpCmdRequest(PtpIpPacket):
    ...
    def __init__(self, data=None, cmd=None, param1=None, param2=None, param3=None, param4=None,
        ...
        if self.param1 is not None:
            self.args = self.args.encode() + struct.pack('L', self.param1)
    ...
    def data(self):
        #print(type(self.cmdtype))
        #print(type(self.unkown))
        #print(type(struct.pack('H', self.ptp_cmd)))
        #print(type(self.transaction_id))
        #print(type(self.args))
        if (type(self.args) == 'str') and not (type(struct.pack('H', self.ptp_cmd)) == 'str'):
            return self.cmdtype + self.unkown + struct.pack('H', self.ptp_cmd) + \
            self.transaction_id + self.args.encode()
        elif (type(self.args) == 'str') and (type(struct.pack('H', self.ptp_cmd)) == 'str'):
            return self.cmdtype + self.unkown + struct.pack('H', self.ptp_cmd).encode() + \
            self.transaction_id + self.args.encode()
        elif ((type(struct.pack('H', self.ptp_cmd)) == 'str')) and not (type(self.args) == 'str'):
            return self.cmdtype + self.unkown + struct.pack('H', self.ptp_cmd).encode() + \
